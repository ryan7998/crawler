...
// runId is the identifier for the current crawl run (generated by the controller).
async function ensureProcessor(crawlId, runId) {
  ...
  const io = getSocket();
  const crawlIdStr = String(crawlId);
  ...
  // Scope the "already processed" counter to the current runId so that data
  // from previous completed runs does not falsely satisfy the completion check.
  // If no runId was supplied (e.g. called from a legacy path) fall back to 0.
  const alreadyProcessed = runId
    ? (await CrawlData.distinct('url', { crawlId, runId })).length
    : 0;

  // If all URLs for this run are already done AND there are no waiting jobs,
  // this is a genuine post-crash recovery — finalise without processing again.
  const waitingCount = await q.getWaitingCount();
  if (alreadyProcessed >= total && waitingCount === 0) {
