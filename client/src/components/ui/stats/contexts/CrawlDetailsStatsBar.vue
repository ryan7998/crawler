  ...
  <div class="flex items-center justify-between w-full">
    ...
    <div class="flex items-center space-x-8">
      <div class="flex items-center space-x-2">
        ...
        <span class="text-sm text-gray-600">Crawl: 
          <span class="font-semibold text-gray-900">{{ contextData.title || 'Loading...' }}</span>
        </span>
      </div>
      <div class="flex items-center space-x-2">
        <div class="w-3 h-3 bg-green-500 rounded-full"></div>
        <span class="text-sm text-gray-600">Status: 
          <span class="font-semibold text-gray-900">{{ contextData.status || 'Loading...' }}</span>
        </span>
      </div>
      <div class="flex items-center space-x-2">
        <div class="w-3 h-3 bg-indigo-500 rounded-full"></div>
        <span class="text-sm text-gray-600">URLs: 
          <span class="font-semibold text-gray-900">{{ contextData.totalUrls || 0 }}</span>
        </span>
      </div>
      <div class="flex items-center space-x-2">
        <div class="w-3 h-3 bg-purple-500 rounded-full"></div>
        <span class="text-sm text-gray-600">Completed: 
          <span class="font-semibold text-gray-900">{{ contextData.completedUrls || 0 }}</span>
        </span>
      </div>
      <div class="flex items-center space-x-2">
        <div class="w-3 h-3 bg-yellow-500 rounded-full"></div>
        <span class="text-sm text-gray-600">Failed: 
          <span class="font-semibold text-gray-900">{{ contextData.failedUrls || 0 }}</span>
        </span>
      </div>
    </div>
    ...
    <div class="flex items-center space-x-3">
      ...
      >
        ...
        {{ contextData.status === 'pending' ? 'Start' : 'Restart' }}
      </button>
    </div>
  </div>
</template>
